...
const groupSettings = JSON.parse(fs.readFileSync(groupSettingsFile));
...
app.get('/', async (req, res) => {
    if (!lastQR) return res.send('<h2>QR code not generated yet. Start the bot first!</h2>');
    const qrImg = await qrcode.toDataURL(lastQR);
    res.send(`
        <h2>Scan QR to Pair Your WhatsApp Number 💞</h2>
        <img src="${qrImg}" width="300" height="300" />
        <p>After scanning, refresh the page if needed.</p>
    `);
});
...
startBot();
console.log('☠️ WhatsApp BOT RUNNING WITH FULL MENU 💞');